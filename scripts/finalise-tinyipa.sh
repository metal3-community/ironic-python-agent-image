...
cp "${WORKDIR}/build_files/ntpdate" "${FINALDIR}/bin/ntpdate"
chmod 755 "${FINALDIR}/bin/ntpdate"
...
chown "${TC}:${STAFF}" "${FINALDIR}/usr/local/tce.installed"
...
cp "${WORKDIR}/build_files/bootlocal.sh" "${FINALDIR}/opt/."

# Copy udhcpc.script to opt
cp "${WORKDIR}/udhcpc.script" "${FINALDIR}/opt/"

# Replace etc/init.d/dhcp.sh
cp "${WORKDIR}/build_files/dhcp.sh" "${FINALDIR}/etc/init.d/dhcp.sh"
sed -i "s/%UDEV_SETTLE_TIMEOUT%/${TINYIPA_UDEV_SETTLE_TIMEOUT}/" "${FINALDIR}/etc/init.d/dhcp.sh"

# Disable ZSwap
sed -i '/# Main/a NOZSWAP=1' "${FINALDIR}/etc/init.d/tc-config"
# cp $WORKDIR/build_files/tc-config $FINALDIR/etc/init.d/tc-config

# Place ipv6 modprobe config so the kernel support loads.
cp "${WORKDIR}/build_files/modprobe.conf" "${FINALDIR}/etc/modproble.conf"
...
find "${FINALDIR}" -path "*babel/locale-data/*.dat" -not -path "*en_US*" -print0 | xargs -0 --no-run-if-empty rm
